(function (f) {
    ...
    function n(a, b) {
        ...
        this.host = {pro: "//catfront.dianping.com", dev: "//catfront.51ping.com"}[this.env];
        var c = this;
        setTimeout(function () {
            c.sendBatch()
        }, 500)
    ...
    function F(a, b, c) {
        ...
        try {
            var C = Date.now(), h = new f.XMLHttpRequest;
            if ("withCredentials" in h) {
                h.open("post", a, !0);
                if (d) for (var g in d) d.hasOwnProperty(g) && h.setRequestHeader(g, d[g]);
                h.onload = function () {
    ...
    var l = document, q = /mobile|iPhone|Android|htc|Lenovo|huawei/i.test(f.navigator.userAgent.toString());
    f.Yoda = {};
    f.YODA_CONFIG = {};
    ...
    e.prototype.getConf = function (a) {
        ...
        a = {requestCode: a, feVersion: this.feVersion, source: this.source};
        var c = this, d = Date.now();
        F(b, a, function (a) {
            a = JSON.parse(a);
            var f = Date.now() - d, h = {
                kvs: {pagedata: [f], TTFB: [f]},
                tags: {
    ...
    e.prototype.isNeedLoad = function () {
        var a = l.getElementsByTagName("script"), b = a.length, c = 0;
        if (m && k && b) for (; c < b; c++) {
            var d = a[c].src;
            ~d.indexOf(m) && (u = !1, t = !0);
            ~d.indexOf(k) && (v = !1, r = !0);
        ...
        var a = this, b = function () {
            function b(b) {
                return function (c, d, g) {
                    var h = Date.now(), e = l.createElement(b), p = l.head, C = "src", k = "link" === b ? "css" : "js";
                    e.onload = function () {
            ...
            return {css: b("link"), js: b("script")}
        }();
        u && (y.report("yoda", "req"), b.js(A, "yoda", function () {
            t = !t;
            a.moduleInit()
        }));
        if (v) if (y.report(g, "req"), "withCredentials" in new f.XMLHttpRequest) b.js(w, g, function () {
            r = !0;
            a.moduleInit()
    ...
    e.prototype.handleError = function (a, b) {
        ...
        if (c && "function" === typeof f[c]) {
            var d = {code: "121333", requestCode: this.options.requestCode};
            setTimeout(function () {
                    f[c](d)
                },
                1E3)
    ...
    e.prototype.filter = function () {
        ...
        if (a[0] && 1 === a.length) D = this.config.riskLevel; else {
    ...
    x.prototype.report = function (a, b, c) {
        ...
        a = {nm: "MV", tm: Date.now(), nt: 0, isauto: 6, val_cid: a, val_bid: b, val_lab: c};
        this.addToSendQueue(d,
            a)
    };
    x.prototype.addToSendQueue = function (a, b) {
        var c;
        (c = this.sendQueue[a.channel]) ? c = c.data : (c = [], this.sendQueue[a.channel] = {conf: a, data: c});
        c.push(b);
        var d = this;
        setTimeout(function () {
            d.send()
    ...
    x.prototype.send = function () {
        var a = [];
        for (d in this.sendQueue) if (this.sendQueue.hasOwnProperty(d)) {
            var b = this.sendQueue[d];
            var c = b.conf;
            (b = H(c, {appnm: c.appnm, category: "data_sdk_" + d, evs: b.data})) && a.push(b)
        }
        if (0 < a.length) {
            var d = "https://report.meituan.com/?_lxskd_rnd=" + Date.now() + Math.ceil(1E3 * Math.random());
            this.sendStatic(d, {data: a})
        }
        this.sendQueue = {}
    };
    x.prototype.sendStatic = function (a, b) {
        ...
        try {
            ...
            if ("withCredentials" in c) c.open("POST", a, !0); else if ("undefined" !== typeof f.XDomainRequest) c = new f.XDomainRequest, c.open("POST", a); else throw Error("\u7075\u7280\u521b\u5efaXHR\u5bf9\u8c61\u5931\u8d25");
            c.onerror = function () {
                c.abort();
                c = null
            };
    ...
    n.prototype.speed = function (a, b) {
        ...
        var d = this, e = new f.XMLHttpRequest;
        e.open("GET", c);
        e.onerror = function () {
            d.speed(a, b);
            e.abort();
    ...
    n.prototype.sendLog = function (a, b, c, d) {
        ...
        this.sendStatic(this.host + "/api/log?v=" + this.catVersion, {c: JSON.stringify(a)})
    };
    n.prototype.metric = function (a) {
        this.sendStatic(this.host + "/api/metric?v=" + this.catVersion + "&p=" + this.project, {data: JSON.stringify(a)})
    };
